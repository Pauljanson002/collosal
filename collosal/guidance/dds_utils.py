...
class StableDiffusion(nn.Module):
    ...
    def get_text_embeds(self, prompt):
        ...
        return embeddings
    def pred_noise_map(self, latents, noise, t, guidance_scale, text_embeddings):
    ...
    def train_step(self, text_embeddings, pred_rgb, ref_rgb, guidance_scale=100, as_latent=False, grad_scale=1,save_guidance_path=None):
        ...
        if as_latent:
            ...
            ref_rgb_512 = F.interpolate(ref_rgb, (512, 512), mode='bilinear', align_corners=False)
            # encode image into latents with vae, requires grad!
            latents_pred = self.encode_imgs(pred_rgb_512)
            latents_ref = self.encode_imgs(ref_rgb_512)

        # timestep ~ U(0.02, 0.98) to avoid very high/low noise level
        t = torch.randint(self.min_step, self.max_step + 1, (latents_pred.shape[0],), dtype=torch.long, device=self.device)
        ...
        # w(t), sigma_t^2
        w = (1 - self.alphas[t])
        grad = grad_scale * w[:, None, None, None] * (noise_pred_pred - noise_pred_ref)
        grad = torch.nan_to_num(grad)

        if save_guidance_path:
            with torch.no_grad():
                if as_latent:
                    pred_rgb_512 = self.decode_latents(latents_pred)
                ...
                alphas = self.scheduler.alphas.to(latents_pred)
                total_timesteps = self.max_step - self.min_step + 1
                index = total_timesteps - t.to(latents_pred.device) - 1 
                b = len(noise_pred_pred)
        ...
        loss = SpecifyGradient.apply(latents_pred, grad)
        # targets = (latents_pred - grad).detach()
        # loss = 0.5 * F.mse_loss(latents_pred.float(), targets,reduction="sum") / latents_pred.shape[0]
        return loss
    def pred_noise_map_perpneg(self, latents, noise, t, guidance_scale, text_embeddings, weights,  B ,K):
        latents_noisy = self.scheduler.add_noise(latents, noise, t)
        # pred noise
    ...
    def train_step_perpneg(self, text_embeddings, weights, pred_rgb, ref_rgb, guidance_scale=100, as_latent=False, grad_scale=1,
                           ):
        ...
        with torch.no_grad():
            # add noise
            noise = torch.randn_like(latents_pred)
            noise_pred_pred = self.pred_noise_map_perpneg(latents_pred,noise,t,guidance_scale,text_embeddings,
                                                          weights,B,K)
            noise_pred_ref = self.pred_noise_map_perpneg(latents_ref, noise, t, guidance_scale, text_embeddings,
                                                          weights, B, K)
            # import kiui
        # latents_tmp = torch.randn((1, 4, 64, 64), device=self.device)
        # latents_tmp = latents_tmp.detach()
...
if __name__ == '__main__':
    ...
    imageio.imsave('./horse_sd.png',imgs[0])
